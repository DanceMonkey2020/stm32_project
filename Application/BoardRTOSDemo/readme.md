...
本项目在[STM32工程搭建 (Cursor + GCC + MinGW + CMake + OpenOCD)](https://www.yulc.fun/?articleId=4)基础上，增加**V11.1.0**最新版本的支持



## 下载源码

下载FreeRTOS内核版本 **V11.1.0**
...
**V11.1.0**版本的目录结构与之前有明显的不同
需要的核心文件全部在在FreeRTOS-Kernel中

`FreeRTOSv202406.01-LTS->FreeRTOS-LTS->FreeRTOS->FreeRTOS-Kernel`

复制已下文件到项目中并在CMakeLists.txt中引用
...
引入配置文件FreeRTOSConfig.h，并修改

以下为修改处(修改为适配各自的芯片及板子)
...
### 编写Application/CMakeLists.txt新增测试样例目录
```
#FreeRTOS
...
### Application/BoardRTOSDemo/main.c试运行
